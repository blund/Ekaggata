...
void setup_graphics (Render_Context *context, int width, int height, int emulated_width, int emulated_height) {
  ...
  context->display  = SDL_CreateTexture(context->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, emulated_width, emulated_height);
...
void render (Render_Context *context, CPU *cpu) {
  ...
  SDL_UpdateTexture(context->display, NULL, cpu->framebuffer, context->emulated_width*sizeof(uint32_t)); // @Merk - ganger her med bredden av skjermen (bytePitch)
